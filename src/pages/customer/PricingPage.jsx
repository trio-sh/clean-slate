...
const PricingPage = () => {
  ...
  // Category icons mapping (keep static)
  const categoryIcons = {
    'shirts-blouses': '👔',
    'pants-shorts': '👖',
    'skirts': '👗',
  ...
  // Get laundry pricing from database (fallback to hardcoded if not found)
  const washFoldRegular = services.find(s => s.slug === 'wash-fold-regular' || s.name.toLowerCase().includes('wash') && s.name.toLowerCase().includes('fold'));
  ...
  }).filter(cat => cat.items.length > 0); // Only show categories with services

  // Fallback categories if database is empty (for backwards compatibility)
  const fallbackPricingCategories = [
    {
      id: 'shirts',
  ...
  const displayCategories = pricingCategories.length > 0 ? pricingCategories : fallbackPricingCategories;
